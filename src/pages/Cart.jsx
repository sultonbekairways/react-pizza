...
import { clearCart, clearAlike, minusPizza, plusPizza } from '../redux/actions/cart';
import emptyImg from '../assets/img/empty-cart.png';

function Cart() {
    const dispatch = useDispatch()
    // @ts-ignore
    const { items, totalPrice, totalCount } = useSelector(data => data.cart)

    const clear = () => {
        if (window.confirm('Всё очистить? Может не будешь ломаться и закашешь по быстрее! А то у нас с доставкой беда! ^_^'))
            dispatch(clearCart())
    }

    const handleClearAlike = (index) => {
        if (window.confirm('Are u sure about that?!'))
            dispatch(clearAlike(index))
    }

    const handleMinus = (index) => {
        dispatch(minusPizza(index))
    }

    const handlePlus = (index) => {
        dispatch(plusPizza(index))
    }
            ...
            <div className="cart">
                <div className="cart__top">
                    <h2 className="content__title"><svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        Корзина</h2>
                    <span onClick={clear} className="cart__clear">
                ...
                <div className="content__items">
                    {
                        items.map((item, index) => <CartItem key={`${item.id}_${item.type}_${item.size}`} index={index} handleClearAlike={handleClearAlike} handleMinusFunc={handleMinus} handlePlusFunc={handlePlus} {...item} />)
                    }
                </div>
